...
const ContasRecorrentes: React.FC<ContasRecorrentesProps> = ({
  ...
  const { assinaturas, loading, error, deleteAssinatura, updateAssinatura } = useAssinaturas();

  if (loading) {
    return <div>Carregando contas recorrentes...</div>;
  }
  if (error) {
    return <div>Ocorreu um erro: {error.message}</div>;
  }

  const handleToggle = async (assinatura: Assinatura) => {
    const hoje = new Date();
    const proximoPagamento = new Date(assinatura.proximo_pagamento + 'T12:00:00');
    let newDate;

    if (proximoPagamento > hoje) {
      // Já está pago, então reverte para o ciclo anterior
      newDate = new Date(proximoPagamento);
      ...
      // Não está pago, avança para o próximo ciclo
      newDate = new Date(proximoPagamento);
       if (assinatura.ciclo === 'MENSAL') {
        newDate.setMonth(newDate.getMonth() + 1);
      } else {
    ...
    await updateAssinatura(assinatura.id, { proximo_pagamento: newDate.toISOString().split('T')[0] });
  };

  const totalAnual = assinaturas.reduce((acc, a) => {
    return acc + (a.ciclo === 'MENSAL' ? a.valor * 12 : a.valor);
  }, 0);
        ...
        {assinaturas.length === 0 ? (
          ...
          <div className="space-y-4">
            {assinaturas.map((a, index) => {
              const pagoEsteMes = new Date(a.proximo_pagamento + 'T12:00:00') > new Date();
              return (
                <div key={a.id} className="p-4 rounded-2xl border bg-white hover:shadow-lg transition-shadow dark:bg-slate-800 dark:border-slate-700/60 animate-fadeInUp" style={{ animationDelay: `${index * 25}ms` }}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                        <span>Próximo vencimento em {new Date(a.proximo_pagamento + 'T12:00:00').toLocaleDateString('pt-BR')}</span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between gap-2 mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                    <div>
                      {pagoEsteMes ? (
                        <button onClick={() => handleToggle(a)} className="text-xs font-medium flex items-center gap-2 px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-lg dark:bg-emerald-500/10 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-500/20 transition-colors">
                          <div className="w-2 h-2 bg-emerald-500 rounded-full"></div> Pago
                        </button>
                      ) : (
                        <button onClick={() => handleToggle(a)} className="text-xs font-medium px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 transition-colors">
                          Marcar como Pago
                        </button>
                      )}
                    </div>
